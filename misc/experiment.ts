...
type Stream<T> = Array<T>

type F<A> = A extends Array<infer T> ? Freshable<F<T>> : Freshable<A>;



let counter = -1;
function fresh<T>(): F<T> {
    counter++;
    return {kind: 'Fresh', n: x};
}

function ground<T>(value: F<T>): F<T> {

    
}

function append<T>(): Generator<[F<T[]>, F<T[]>, F<T[]>]> {
    const a = ground([fresh<T>()]);
    const b = fresh();
    yield []
}
